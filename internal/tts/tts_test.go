...
func TestAutomaticPitchCurveKeepsLowSourceIntonationBlend(t *testing.T) {
	cfg := Config{ApplyPitch: true, IntonationStrength: 1}
	if got := rendererIntonationStrength(cfg, &render.PitchCurve{FrameMS: 10, Cents: []float64{0, 20}}); got != automaticSourceIntonationBlend {
		t.Fatalf("renderer intonation strength = %.2f, want %.2f", got, automaticSourceIntonationBlend)
	}
	if got := rendererIntonationStrength(cfg, nil); got != 1 {
		t.Fatalf("source intonation strength = %.2f, want 1", got)
	}
	if got := rendererIntonationStrength(Config{ApplyPitch: false, IntonationStrength: 1}, &render.PitchCurve{FrameMS: 10, Cents: []float64{0, 20}}); got != 0 {
		t.Fatalf("disabled pitch renderer intonation strength = %.2f, want 0", got)
	}
}
