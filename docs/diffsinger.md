...
`dsdur`は音節全体の長さを保ったまま、子音と母音の配分へ弱く反映します。話声用のピッチ曲線がある場合は、その曲線を基準にします。`dspitch`に対応する音源では、音源側の微小なピッチ変化を弱く混ぜて急な変化を抑えます。手動ピッチ編集ではこの補正を使いません。先頭の無音分はピッチ曲線の配置時に補正します。
