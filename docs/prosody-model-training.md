...
この手順で生成したモデルJSONの利用条件は、ベースモデルと教師データの条件を引き継ぎます。同梱のJSUT由来モデルを基にした場合は、学術研究、非商用研究、個人利用の範囲でのみ利用・改変・再配布でき、商用利用にはJSUT権利者の事前許諾が必要です。その場合は、モデルJSONの`license_notice`で指定した文書と`licenses/JSUT-DATA-AND-LABELS.txt`を一緒に示してください。別のベースモデルや教師データを使う場合は、それぞれの利用条件を確認し、適用される文書を`licenses/`に用意してください。
...
1. GUIの「設定」→「設定...」で「開発者モード」を有効にし、「開発者向け」設定の「抑揚の教師データ生成を表示する」も有効にします。
...
評価したモデルJSONを`models/`へコピーします。配布対象にするモデルには、利用条件を示す`license`と、配布物のルートからの相対パスで`licenses/`以下の詳細文書を示す`license_notice`を必ず含めてください。参照先の文書はリポジトリの`licenses/`に用意し、モデルと一緒に配布します。現在の公式同梱モデルはJSUT由来のため、`licenses/PROSODY-MODELS.txt`を使用します。別のベースモデルを使う場合は、そのモデル自身の利用条件と、派生モデルに適用される条件を確認してください。
