...
ZIP内のフォルダ構成は変更しないでください。`runtime`やモデルだけを別の場所へ移動すると、音声を合成できません。

WindowsとmacOSでは、利用可能なネイティブのマルチメディアバックエンドを使用します。Qt Multimedia用の外部FFmpegバックエンドを使う場合は、設定画面でプラグインとコーデックのフォルダを指定してください。初回起動時は、`UTAUTTS_FFMPEG_PATH`、`FFMPEG_PATH`、`FFMPEG_DIR`、`FFMPEG_ROOT`の順に環境変数を確認します。
...
アプリ内アップデーターは通常、安定版だけを更新候補にします。設定画面の「プレリリース版も確認する」を有効にした場合だけ、プレリリース版も更新候補に含めます。プレリリース版も通常リリースと同じ`vMAJOR.MINOR.PATCH`形式で判定します。

更新では`voice/`、`Resamplers/`、`Wavtools/`、`Dependencies/`、`config.ini`と、引き継ぎ可能なユーザーRenderer定義を保持します。Renderer定義は`renderer/<id>/renderer.json`へ配置してください。ZIPを手動で上書きせず、アプリ内アップデーターを使用してください。
...
GUI版には「足立レイ UTAU音源 ver3.5.0」を初期音源として同梱しています。利用条件は[同梱音源](voicebank.md)と音源内の文書を確認してください。
