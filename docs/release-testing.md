...
GUIの手動確認では、更新通知を有効にした状態で安定版だけが候補になること、開発者モードを有効にしたときだけ「開発者向け」設定が表示されること、そこで「プレリリース版も確認する」を有効にしたときだけプレリリース版が候補になることを確認します。また、多言語／phonemizer機能と抑揚教師データ生成を個別に無効化したとき、それぞれのUIが隠れることを確認します。同じ数値バージョンの安定版とプレリリース版がある場合は安定版を選びます。
