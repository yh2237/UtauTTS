# 日本語・英語・中国語の読み上げ

UtauTTSは日本語、英語、中国語に対応しています。音源の収録形式に合う言語と発音形式を選んでください。発音形式は、入力した読みを音源の原音名へ変換する処理です。英語と中国語は音源の収録内容によって発音の明瞭さが変わります。

## 言語と発音形式を選ぶ

| 言語 | `language` | `phonemizer` | 入力 |
| --- | --- | --- | --- |
| 日本語 | `ja` | `ja-kana`（既定） | 日本語文章またはかな読み |
| 英語 | `en` | `en-arpasing`（既定） | 英文またはARPAbet読み |
| 英語 | `en` | `en-delta` | 英文またはARPAbet読み |
| 英語 | `en` | `en-vccv` | 英文またはARPAbet読み |
| 英語 | `en` | `en-cv` | 英文またはARPAbet読み（C+V音源） |
| 中国語 | `zh` | `zh-cvvc`（既定） | 中国語文章または声調付きPinyin |

GUIでは音源と言語を選びます。発音形式の`自動`は音源のメタデータと原音名から収録形式を判定します。手動で固定する場合は発音形式のプルダウンから選びます。DiffSinger音源については[DiffSinger](diffsinger.md)を参照してください。

CLIでは次のように指定します。音源パスとphonemizerは使用する音源に合わせて変更してください。

```powershell
utautts-cli --voicebank "./voice/english-bank" --language en --phonemizer en-arpasing --text "Hello world." --renderer waveform --out hello.wav
```

HTTP APIでは`language`と`phonemizer`を指定します。詳細は[Serverの合成API](server.md)を参照してください。

## 読みを指定する

自動読みが誤る場合は読みを直接指定するか[辞書設定](settings.md#辞書設定)で修正します。CLIでは`--text`の代わりに`--reading`を指定できます。

| 言語 | 読みの例 | 区切り |
| --- | --- | --- |
| 英語 | `HH AH0 L OW1 \| W ER1 L D \| SP` | 空白は音素区切り。`\|`は単語境界。`SP`は休止 |
| 中国語 | `ni3 hao3 \|` | 空白は音節区切り。`\|`は休止 |

英語の母音末尾の数字は強勢を表し、`0`は無強勢、`1`は第一強勢です。中国語の数字は声調を表します。テキスト入力では句読点を休止へ変換します。中国語の空白だけでは休止を入れません。

## 自動読みと韻律

英語はユーザー辞書・音源辞書・同梱CMUdictの順で読みを探します。未登録の所有格・複数形・過去形・ing形は辞書にある語幹から補い、`un-`・`re-`・`micro-`など一部の接頭辞も既知の語幹と組み合わせます。辞書由来の強勢は保持します。

読みが見つからない語は内蔵の綴り規則で推定します。子音の組み合わせや語末の無音のeなどを扱いますが、強勢は未確定になります。固有名詞・略語・不規則な綴りは読みを指定するか辞書で修正してください。

数字・小数・桁区切り・時刻・百分率の読みを展開します。日付や通貨の読みと同綴異音語の文脈判定は網羅していません。

英語の長さは強勢・子音の種類・句末に応じて調整します。音高変化にはピッチ処理の有効化と0より大きい抑揚の強さが必要です。文の意味に応じた強調は扱いません。

Delta・VCCV音源では、無強勢の`AH0`に弱母音の原音を優先します。対応する原音がない場合は通常の`AH`を候補に残します。強勢が未指定の`AH`を自動で弱母音には変えません。語末と次の語頭をまとめた接続音がない場合は、語末子音群の録音も探します。

C+V音源ではARPAbetの各音素を1つの原音へ割り当てます。aliasは文頭の母音が`-V`→`- V`→`V`、文中の母音が`-V`→`V`→`- V`、文頭の子音が`- C`→`-C`→`C`、文中の子音が`C`→`-C`→`- C`の順で探します（`-V`・`-C`は空白なし表記）。語末は母音に`V -`／`V-`、子音に`C -`を使います。この優先順はOpenUtauのEnglish C+V Phonemizer（Cadlaxa）に準拠します。

英語の機能語は、句の途中で前後が休止でなく強調もされない場合に弱形で読みます（E1）。たとえば句中の`of`は無強勢の`AH0 V`になります。単独・句の先頭・末尾では入力どおりに読み、強く読みたい場合は`AH1 V`のように読みを指定してください。ユーザー辞書と音源辞書の指定は弱形化より優先します。

中国語は小規模な内蔵語辞書で「银行」「重庆」「音乐」などを読み分けます。未登録語は文字単位で読みを推定します。数値と小数は読みへ展開し、年の数字は一桁ずつ読みます。「一」「不」と第三声の変調に加えて、軽声の長さと音高を調整します。複雑な文脈での多音字や第三声連続は正しく処理できない場合があります。

中国語の`ü`は読みで`v`または`u:`と書けます。`nüe/lüe`の原音名には`nve/lve`と`nue/lue`の両表記を探し、`presamp.ini`の母音・子音分類もこの表記差を吸収します。`nu/lu`と`nv/lv`は別の母音として扱います。

英語・中国語の抑揚には日本語用モデルを適用せず、規則に基づいて処理します。

## 音が欠ける場合

UtauTTSで生成できるのは、基本的に音源へ収録されている子音と接続音です。音が欠ける場合は、音源と発音形式の組み合わせを確認してください。[読み上げ品質の評価](../tools/evaluation/README.md)では、原音候補と必須音の不足を調べられます。

CPU版WORLDは、Delta・VCCV英語の必須語末子音について、原音後半が再生時間内に収まるよう補正します。対象は発音データから判定するため、音高などの接尾辞が付いたaliasにも対応します。子音群では、次の原音との重なりで子音が消えないように再生時間を確保します。この処理には本体とWORLDブリッジの両方が必要です。任意のrelease音とARPAsing音源は対象外です。

原音の接続位置の補正は[発話タイミング補正](speech-quality-experiment.md)を参照してください。[選択した原音の確認](../tools/evaluation/README.md#選択した原音を確認する)では切り出し範囲と合成結果を比較できます。

音素表記の参考は[OpenUtauのVCCV対応表](https://github.com/stakira/OpenUtau/blob/master/OpenUtau.Plugin.Builtin/EnglishVCCVPhonemizer.cs)、[OpenUtauのC+V対応表](https://github.com/stakira/OpenUtau/blob/master/OpenUtau.Plugin.Builtin/EnglishCpVPhonemizer.cs)と[香港理工大学のPinyin綴り規則](https://www.polyu.edu.hk/bepth/introduction-to-phonetics/spelling-rules-in-pinyin/?sc_lang=en)を参照してください。
