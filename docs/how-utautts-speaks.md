...
下のグラフは、前後ともモーラ長180ms、先行発声80ms、オーバーラップ40msとした例です。線の座標は、Classic faithfulの5点包絡線を作る式から計算しています。実際の形はoto.iniとモーラ長によって変わります。

![5点包絡線による原音接続](assets/how-utautts-speaks-envelope.svg)

OpenUTAU Classic faithful Rendererでは、一つの原音を$p_0$から$p_4$までの5点で扱います。$p_0$では音量0から始まり、$p_1$で1になります。$p_2$から$p_3$まではそのまま鳴らし、$p_4$へ向かって再び0まで下げます。各点の位置は固定ではなくoto.iniと前後のモーラ長から決まります。

時刻$t$での原音$i$の波形を$x_i$、包絡線を$e_i$、配置時刻を$B_i$、接続位置の微調整を$\delta_i$とします。最終的な波形$y(t)$は概念上は次の足し算です。

$$
y(t)=\sum_i e_i(t-B_i-\delta_i)\,x_i(t-B_i-\delta_i)
$$

原音を決めた時刻へ置き、包絡線で音量を出し入れしながら足しています。式にするとなんか長いですが、処理としてはそれだけです。

Classic faithfulでは、重なった原音を数サンプルずつ前後へ動かし、波の山と谷がなるべく合う位置も探します。ずれを$d$としたとき、二つの波形$a$と$b$の似かたを測る正規化相関は

$$
\rho(d)=
\frac{\sum_n a[n]b[n+d]}
{\sqrt{\sum_n a[n]^2}\sqrt{\sum_n b[n+d]^2}}
$$

となります。限られた範囲で$\rho(d)$が最大になる$d$を$\delta_i$として採用します。話すタイミングまで変えるほどの移動ではなく、重なった波が打ち消し合って音量が震えるのを減らすための、かなり小さな調整です。
...
Classic faithfulではWorldlineが原音を分析してから指定した長さと高さで再合成します。Goだけで動くwaveform Rendererは可変レートのリサンプリングとWSOLAで同じ目的を実現しています。どちらも単純な早送りやスロー再生ではありません。
