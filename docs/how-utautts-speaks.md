...
VCVのoto.iniは長い録音を前提にした値になりやすいため、短いモーラへ置くときは先行発声とfixedをそのまま使いません。子音の遷移を残しながら母音末尾を確保し、停止音の閉鎖区間を接続scoreが過剰に悪く評価しないようにしています。
...
同じ録音WAV内で次の原音へ進む候補には連続性の加点があります。ただし離れた位置へ飛ぶ候補は加点を減らすため、同じファイルという理由だけで不自然な組み合わせを選びにくくしています。
...
標準のframe-intonation-v8が予測するのは抑揚だけで、モーラ長は予測しません。通常はGUIで指定した基準モーラ長を使い「ん」は0.9倍、「ー」は1.2倍にします。「っ」は通常のモーラと同じ基準長です。
...
Classic UTAUでは選択したresamplerが原音を指定した長さと高さへ変換します。Goだけで動くwaveform Rendererは可変レートのリサンプリングとWSOLAで同じ目的を実現しています。どちらも単純な早送りやスロー再生ではありません。
...
waveformとClassic UTAUは「加工済みの原音を最後に重ねる」方式。WORLDLINE-Rと既定のUtauTTS WORLD phraseは「原音の特徴を並べてから、最後に一つの声へ戻す」方式です。元になる原音とタイミングは共通なので、Renderer同士の差は小さく聞こえることもあります。
