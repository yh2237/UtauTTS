...
標準のframe-intonation-v9-tの予測対象: 抑揚。モーラ長: GUIで指定した基準モーラ長を使い、「ん」は0.9倍、「ー」は1.2倍。「っ」は通常のモーラと同じ基準長です。

モーラ長はモデルでは予測せず、基準モーラ長と言語別の時間規則で決めます。GUIで手動指定した長さがある場合はその値を優先します。
...
標準のframe-intonation-v9-tモデルが出力するのは音声ではなく、10ミリ秒ごとの高さです。下のグラフは「こんにちは。」を入力し、基準モーラ長120ms、抑揚の強さ1.0で実際に予測した62点を描いています。「ん」はモデル入力前の長さ規則により108msです。
