...
画面上の編集方法は[GUIの使い方](gui.md#イントネーションとモーラ長)を参照してください。JSONで指定していないモーラの補正値は0 centです。
...
手動ピッチの適用条件: `--apply-pitch`。手動カーブは10ms刻みへ補間され、急激な変化は安全制約で滑らかに制限されます。モーラ長の入力: CLIの`--mora-durations`へ読みのモーラ順の配列、または`{"mora_durations_ms": [...]}`。`mora_positions_ms`: プレビューや合成結果に含まれる出力値。
