...
文頭が欠ける音源では「文頭の長さ」を長くし、余計なノイズを拾う音源では短くしてください。新規作成したカードに使う既定値は「設定」→「設定...」から変更できます。初期状態ではモーラ長120 ms、休止長180 ms、抑揚モデル`frame-intonation-v8`、Renderer`openutau-worldline-r-faithful`です。
