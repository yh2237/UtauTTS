...
WORLD rendererの`jsut-cv-transition-tcn-v1`は、単独音CVの母音から次の子音へ移る短い区間を補正するモデルです。単独音の境界だけに使い、推定値が大きい場合は適用しません。モデルは`renderer/utautts-world-phrase/models/`に含まれます。
