...
type darwinLibrary struct{ pointer *C.NativeLibrary }
...
func openNativeLibrary(path string) (nativeLibrary, error) {
	...
	return &darwinLibrary{pointer: pointer}, nil
}
...
func (library *darwinLibrary) Close() error {
...
func darwinNativeRequest(request nativeRequest) (C.SynthRequest, []unsafe.Pointer) {
...
func (library *darwinLibrary) copyAndFree(output *C.float, length int) []float32 {
...
func (library *darwinLibrary) PhraseNew() (uintptr, error) {
...
func (library *darwinLibrary) PhraseDelete(phrase uintptr) {
	C.native_phrase_delete_handle(library.pointer, C.uintptr_t(phrase))
}

func (library *darwinLibrary) PhraseAdd(phrase uintptr, index int, request nativeRequest, timing phraseTiming) error {
	native, allocations := darwinNativeRequest(request)
...
func (library *darwinLibrary) PhraseSetCurves(phrase uintptr, f0, gender, tension, breathiness, voicing []float64) error {
...
func (library *darwinLibrary) PhraseSynth(phrase uintptr) ([]float32, error) {
	var output *C.float
